...
class Weixinpay {
    ...
    /**
     * 统一下单
     * $order 订单 必须包含支付所需要的参数 body(产品描述)、total_fee(订单金额)、
     * out_trade_no(订单号)、product_id(产品id)、trade_type(类型：JSAPI，NATIVE，APP)
     * @param $order
     * @return array
     */
    public function unifiedOrder($order, $b_id){
        // 获取配置项
     ...
     */
    public function getParameters($openid, $b_id){
        // 获取配置项
        $config=$this->config;
        ...
        if (!isset($_GET['code'])) {
            ...
            $out_trade_no = time();
            // 返回的url
            $redirect_uri=U('/WeixinPay/pay/'.$b_id,'','',true);
            $redirect_uri=urlencode($redirect_uri);
            $url='https://open.weixin.qq.com/connect/oauth2/authorize?appid='.$config['APPID']
                .'&redirect_uri='.$redirect_uri.'&response_type=code&scope=snsapi_base&state='
                .$out_trade_no.'#wechat_redirect';
            redirect($url);
        }else{ 
            ...
            //$openid='odYPD1eW3LRaG6uIytEwty5X4Qjg';
            // 订单数据  请根据订单号out_trade_no 从数据库中查出实际的body、total_fee、out_trade_no、product_id
            $cartPayController = new \Home\Controller\WeixinPayController();
            $all_order_amount = $cartPayController->wxPayOrder($b_id);
            $total_fee = sprintf("%.2f", $all_order_amount)*100;//floatval($all_order_amount)*100;
            $order=array(
                'body'=>'租房房租',// 商品描述（需要根据自己的业务修改）
