...
class WenController extends AdminController
{
    ...
    public function editWen(){
        $id = i('id');
        $where['id'] = $id;
        $info = M('wen_zhang') -> where($where) -> find();
        if(IS_POST){
            $data = i('post.');
            M('wen_zhang') -> where($where) -> save($data);
            $this -> success('修改成功！');
        }else{
    ...
    public function delWen(){
        ...
        $info = M('wen_zhang')->where($arr)->delete();
        if($info){
            $this -> success('删除成功！');
        }else{
            $this -> error("删除失败！");
