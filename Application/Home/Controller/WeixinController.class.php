...
//            {
//                print_r($this->get_user_info($val));
//            }exit;
            return json_decode($result);
        }
        ...
        //模板消息列表
        public function push_message($msg = array())
        {
            $msg = json_encode($msg);
            $token = $this->getToken();
            $url = 'https://api.weixin.qq.com/cgi-bin/message/template/send?access_token='.$token;
            $result = curl_request($url, $msg, 'POST');
            return json_decode($result);
        }
        
        
        public function wxpay($openid = 'odYPD1eW3LRaG6uIytEwty5X4Qjg')
        {
            //②、统一下单
